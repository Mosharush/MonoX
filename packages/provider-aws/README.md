...
Produces a deterministic Pulumi Automation API intent for AWS targets. It is deliberately plan-only in 0.2.0:
no AWS SDK, Pulumi CLI or cloud endpoint is called. Authentication must be represented by a GitHub OIDC role
reference in `target.bindings.identityRef`; external secret integration is selected by
